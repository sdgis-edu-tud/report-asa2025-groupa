...
Since the green space percentages approximately range from 0% to 30%, the values were normalized by dividing by 30, using the following formula.
...
However, when normalizing, larger distance indicates weaker connectivity between stream unit and green space nearby. Therefore, the values are inverted to ensure that higher normalized scores reflect stronger connectivity. As a result, the following formula was applied:
